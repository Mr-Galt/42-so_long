...
/*   Updated: 2023/11/07 14:39:28 by mheinke          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int mouse_click(int button, int x, int y, t_game *game)
{
	...
	if (game->status->actual_status == CREDITS_MENU)
	{
		if (button == 1 && x >= game->status->credit_back[0] && x <= game->status->credit_back[2] &&
			y >= game->status->credit_back[1] && y <= game->status->credit_back[3])
		{
			game->status->actual_status = START_MENU;
...
int mouse_move(int x, int y, t_game *game) 
{
	...
	if (game->status->actual_status == CREDITS_MENU)
	{
		if (x >= game->status->credit_back[0] && x <= game->status->credit_back[2] &&
			y >= game->status->credit_back[1] && y <= game->status->credit_back[3])
			game->status->creditscreen_status = 1;
		else
		 	game->status->creditscreen_status = 0;
	}
	return 0;
